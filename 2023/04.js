...
function decode(message = '') {
    ...
    message.split('').forEach((element, index) => {
        if (element === '(') open_groups.push(index)
        if (element === ')') {
            const sub = message.slice(open_groups[open_groups.length - 1], index + 1)
            const rep = message.slice(open_groups[open_groups.length - 1] + 1, index)
            if (!sub.match(/\(.*\)/g)) return
            message = message.replace(sub, reverse(rep))
