...
// Each unit of time, the sled moves one position to the right. If it encounters a closed barrier, it stops until the barrier opens. 
...
// https://www.codewars.com/kata/5d0ae91acac0a50232e8a547/javascript
...
function cyberReindeer(road = '', time = 0) {
    ...
    while (deltaTime !== time) {
        ...
        deltaTime++
        if (deltaTime === 5) arrayRoad = arrayRoad.map(element => element === '|' ? '*' : element)

        const index = arrayRoad.findIndex(element => element === 'S')
        if (arrayRoad[index + 1] === '|') {
            console.log('detiene')
        } else if (index < road.length - 1) {
            if (flag) {
                arrayRoad[index] = '*'
                flag = false
            } else arrayRoad[index] = '.'
            if (arrayRoad[index + 1] === '*') flag = true
            arrayRoad[index + 1] = 'S'
        }
    }
    return result
