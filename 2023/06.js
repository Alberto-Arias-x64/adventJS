...
// la distancia es el valor absoluto de la distancia recorrida máxima al finalizar los movimientos.

function maxDistance(movements = '') {
    const right = movements.match(/>/g)?.length | 0
    const left = movements.match(/</g)?.length * -1 | 0
    const com = movements.match(/\*/g)?.length | 0
    const total = Math.abs(right + left) + com
    return total
}
